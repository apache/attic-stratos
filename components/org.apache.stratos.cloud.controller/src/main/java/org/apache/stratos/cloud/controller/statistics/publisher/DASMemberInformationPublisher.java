...
public class DASMemberInformationPublisher extends ThriftStatisticsPublisher implements MemberInformationPublisher {
    ...
    private static StreamDefinition createStreamDefinition() {
        try {
            ...
            payloadData.add(new Attribute(CloudControllerConstants.CPU_COL, AttributeType.INT));
            payloadData.add(new Attribute(CloudControllerConstants.RAM_COL, AttributeType.INT));
        ...
        Runnable publisher = new Runnable() {
            ...
            public void run() {
                ...
                if (metadata != null) {
                    ...
                    payload.add(Integer.valueOf(metadata.getCpu()));
                    payload.add(Integer.valueOf(metadata.getRam()));
                    payload.add(metadata.getImageId());
                    payload.add(Integer.valueOf(metadata.getLoginPort()));
